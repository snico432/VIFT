...
class IMUToVisualCrossAttnPoseTransformer(nn.Module):
    """
...
class CrossAttnPoseTransformer(nn.Module):
    ...
    Pose transformer variant that performs cross-attention:
    - IMU latent tokens first query visual latent tokens
    - an FFN processes the fused IMU representation before self-attention
    - IMU latent tokens then interact through self-attention
    - visual latent tokens are used as keys/values
    """
                ...
                nn.ModuleDict(
                    {
                        ...
                        "ln_cross": nn.LayerNorm(embedding_dim),
                        "ffn_cross": nn.Sequential(
                            nn.Linear(embedding_dim, dim_feedforward),
                            nn.ReLU(),
                            nn.Dropout(dropout),
                            nn.Linear(dim_feedforward, embedding_dim),
                        ),
                        "ln_cross_ffn": nn.LayerNorm(embedding_dim),
                        "self_attn": nn.MultiheadAttention(
                            embed_dim=embedding_dim,
                            num_heads=nhead,
                            dropout=dropout,
                        ...
                        "ln_self": nn.LayerNorm(embedding_dim),
                        "ffn_self": nn.Sequential(
                            nn.Linear(embedding_dim, dim_feedforward),
                            nn.ReLU(),
                            nn.Dropout(dropout),
                            nn.Linear(dim_feedforward, embedding_dim),
                        ),
                        "ln_out": nn.LayerNorm(embedding_dim),
                    }
                )
                for _ in range(num_layers)
            ]
    ...
    def forward(self, batch, gt):
        ...
        for layer in self.layers:
            ...
            imu = layer["ln_cross"](imu + self.dropout(cross_attn_out))

            ffn_cross_out = layer["ffn_cross"](imu)
            imu = layer["ln_cross_ffn"](imu + self.dropout(ffn_cross_out))
            ...
            imu = layer["ln_self"](imu + self.dropout(self_attn_out))

            ffn_self_out = layer["ffn_self"](imu)
            imu = layer["ln_out"](imu + self.dropout(ffn_self_out))

        # (B, S, 6)
        return self.fc2(imu)


class VisualContextCrossAttnPoseTransformer(nn.Module):
    """
    Pose transformer variant that performs two-stream cross-attention:
    - Visual tokens first self-attend (causally) to build richer visual context
    - IMU tokens query the updated visual tokens via cross-attention
    - IMU tokens then self-attend to integrate attended visual information
    - FFN applied to IMU stream; pose is decoded from IMU tokens only
    """
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                nn.ModuleDict(
                    {
                        "visual_self_attn": nn.MultiheadAttention(
                            embed_dim=embedding_dim,
                            num_heads=nhead,
                            dropout=dropout,
                            batch_first=True,
                        ...
                        "ln1": nn.LayerNorm(embedding_dim),
                        "imu_to_visual_cross_attn": nn.MultiheadAttention(
                            embed_dim=embedding_dim,
                            num_heads=nhead,
                            dropout=dropout,
                            batch_first=True,
                        ...
                        "ln2": nn.LayerNorm(embedding_dim),
                        "imu_self_attn": nn.MultiheadAttention(
                            embed_dim=embedding_dim,
                            num_heads=nhead,
                            dropout=dropout,
                            batch_first=True,
                        ...
                        "ln4": nn.LayerNorm(embedding_dim),
                    }
                )
                for _ in range(num_layers)
            ]
    ...
    def forward(self, batch, gt):
        ...
        for layer in self.layers:
            visual_self_attn_out, _ = layer["visual_self_attn"](
                query=visual,
                key=visual,
                value=visual,
                attn_mask=attn_mask,
            ...
            visual = layer["ln1"](visual + self.dropout(visual_self_attn_out))

            imu_to_visual_cross_attn_out, _ = layer["imu_to_visual_cross_attn"](
                query=imu,
                key=visual,
                value=visual,
                attn_mask=attn_mask,
            ...
            imu = layer["ln2"](imu + self.dropout(imu_to_visual_cross_attn_out))

            imu_self_attn_out, _ = layer["imu_self_attn"](
                query=imu,
                key=imu,
                value=imu,
                attn_mask=attn_mask,    
            ...
            imu = layer["ln3"](imu + self.dropout(imu_self_attn_out))

            ffn_out = layer["ffn"](imu)
            imu = layer["ln4"](imu + self.dropout(ffn_out))
